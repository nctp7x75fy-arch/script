...
local function aimAt(target)
  ...
  if Config.PredictionEnabled then
    ...
    targetPos = targetPos + (targetVelocity * Config.PredictionAmount)
  end
  ...
  -- Stronger smoothing with shake reduction
...
local function createMenu()
  ...
  local function createToggle(name, yPos, getValue, setValue)
    local label = Instance.new("TextLabel")
    label.Size = UDim2.new(0.6, -20, 0, 30)
    label.Position = UDim2.new(0, 15, 0, yPos)
    label.BackgroundTransparency = 1
    label.Text = name
    label.TextColor3 = Color3.new(0.9, 0.9, 0.9)
    ...
    toggle.Position = UDim2.new(1, -75, 0, yPos)
    toggle.BackgroundColor3 = getValue() and Color3.fromRGB(50, 200, 100) or Color3.fromRGB(60, 60, 70)
    toggle.BorderSizePixel = 0
    toggle.Text = getValue() and "ON" or "OFF"
  ...
  createToggle("Prediction", 270, 
    function() return Config.PredictionEnabled end,
    function(val) Config.PredictionEnabled = val end
  )
  
  createToggle("Shake Reduction", 310,
    function() return Config.ShakeReduction end,
    function(val) Config.ShakeReduction = val end
  )
  
  createToggle("Team Check", 350,
    function() return Config.TeamCheck end,
    function(val) Config.TeamCheck = val end
  )
  ...
  info.Position = UDim2.new(0, 15, 0, 510)
  info.BackgroundTransparency = 1
  info.Text = "🎯 ENHANCED AIMBOT | Status: [" .. (Config.AimbotEnabled and "ON" or "OFF") .. "] ESP: [" .. (Config.ESPEnabled and "ON" or "OFF") .. "]\nPrediction & Shake Reduction Active | Script by kkavasaki__"
