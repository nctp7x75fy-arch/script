...
local function showMenu()
  ...
  menu.Name = "Menu"
  menu.Size = UDim2.new(0, 350, 0, 420)
  menu.Position = UDim2.new(0.5, -175, 0.5, -210)
  ...
  colorsFrame.Size = UDim2.new(1, -30, 0, 330)
  ...
  colorsFrame.CanvasSize = UDim2.new(0, 0, 0, 500)
  ...
  local function createSlider(labelText, yPos, min, max, initial, decimals, onChange)
    local label = Instance.new("TextLabel")
    ...
    track.Size = UDim2.new(1, -60, 0, 20)
    track.Position = UDim2.new(0, 0, 0, yPos + 25)
    track.BackgroundColor3 = Color3.fromRGB(60, 60, 70)
    track.BorderSizePixel = 0
    track.Parent = colorsFrame
    ...
    knob.Position = UDim2.new(fill.Size.X.Scale, -9, 0.5, 0)
    ...
    valueLabel.Position = UDim2.new(1, 6, 0, yPos + 22)
    valueLabel.BackgroundTransparency = 1
    valueLabel.TextColor3 = Color3.new(0.9, 0.9, 0.9)
    valueLabel.TextSize = 14
    valueLabel.Font = Enum.Font.GothamBold
    valueLabel.TextXAlignment = Enum.TextXAlignment.Left
    valueLabel.Parent = colorsFrame
    
    local dragging = false
    local function setFromPos(absX)
      local relative = math.clamp((absX - track.AbsolutePosition.X) / track.AbsoluteSize.X, 0, 1)
      fill.Size = UDim2.new(relative, 0, 1, 0)
      knob.Position = UDim2.new(relative, -9, 0.5, 0)
      local rawVal = min + (max - min) * relative
      local mult = 10 ^ (decimals or 0)
      local rounded = math.floor(rawVal * mult + 0.5) / mult
    ...
    -- initialize
    local initRel = math.clamp((initial - min) / (max - min), 0, 1)
    fill.Size = UDim2.new(initRel, 0, 1, 0)
    knob.Position = UDim2.new(initRel, -9, 0.5, 0)
    local mult = 10 ^ (decimals or 0)
    valueLabel.Text = tostring(math.floor(initial * mult + 0.5) / mult)
  ...
  -- Info section (now will be updated dynamically)
...
Smoothness: %.2f | Team Check: %s]], 
  ...
  -- Create FOV slider (range 50 - 500)
  local fovSlider = createSlider("FOV Radius", 340, 50, 500, Config.FOVRadius, 0, function(val)
  ...
  -- Create Smoothness slider (range 0.01 - 1, 2 decimals)
  local smoothSlider = createSlider("Smoothness", 380, 0.01, 1, Config.Smoothness, 2, function(val)
  ...
  credits.Position = UDim2.new(0, 0, 0, 460)
